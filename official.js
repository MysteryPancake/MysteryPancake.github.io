...
function addRepo(repo, div) {
	request(repo.url + "/readme", function(result) {
		...
		if (start.startsWith("<img src=\"")) {
			var match = start.match(/src="(.*?)"/);
			if (match) {
				var url = document.createElement("a");
				url.href = repo.homepage || repo.html_url;
				var img = document.createElement("img");
				img.src = repo.html_url + "/blob/master/" + match[1];
				img.width = 72;
				img.height = 72;
				url.appendChild(img);
		...
		description.textContent = repo.description.replace("(work in progress)", "");
		...
		if (repo.has_pages && repo.homepage && repo.homepage.indexOf(window.location.host) !== -1) {
